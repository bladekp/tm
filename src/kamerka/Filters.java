...
public class Filters {
    ...
    void brightness(BufferedImage myImage, int brightness) {
        for (int i = 0; i < myImage.getHeight(); i++) {
            for (int j = 0; j < myImage.getWidth(); j++) {
    ...
    void gamma(BufferedImage myImage, int g) {
    ...
    void inversion(BufferedImage myImage) {
        for (int i = 0; i < myImage.getHeight(); i++) {
            for (int j = 0; j < myImage.getWidth(); j++) {
                Color c = new Color(myImage.getRGB(j, i));
                int red = (int) (c.getRed());
    ...
    void solarise(BufferedImage myImage, int threshold) {
        for (int i = 0; i < myImage.getHeight(); i++) {
            for (int j = 0; j < myImage.getWidth(); j++) {
                Color c = new Color(myImage.getRGB(j, i));
                int red = (int) (c.getRed());
    ...
    void sharpness(String sourcePath, int a, int b, int g) {
    ...
    void border(String sourcePath, int s, int borderType) {
        if (borderType == -1) {
            return;
        }
        if (borderType == 5) {
    ...
    void thresholding(String sourcePath, int thesh, int type) {
        if (type == -1) {
            return;
        }
        System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
    ...
    void flip(String sourcePath, int code){
        if (code == -1) return;
        System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
        Mat source = Highgui.imread(sourcePath, Highgui.CV_LOAD_IMAGE_COLOR);
        Mat destination = new Mat(source.rows(), source.cols(), source.type());
    ...
    void gaussian(String sourcePath, int size){
        try{
        System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
        Mat source = Highgui.imread(sourcePath, Highgui.CV_LOAD_IMAGE_COLOR);
        Mat destination = new Mat(source.rows(), source.cols(), source.type());
    ...
    void erosion(String sourcePath, int size){
        System.loadLibrary( Core.NATIVE_LIBRARY_NAME );
         Mat source = Highgui.imread(sourcePath,  Highgui.CV_LOAD_IMAGE_COLOR);
         Mat destination = new Mat(source.rows(),source.cols(),source.type());
         destination = source;
    ...
    void dilation(String sourcePath, int size){
        System.loadLibrary( Core.NATIVE_LIBRARY_NAME );
         Mat source = Highgui.imread(sourcePath,  Highgui.CV_LOAD_IMAGE_COLOR);
         Mat destination = new Mat(source.rows(),source.cols(),source.type());
         destination = source;
